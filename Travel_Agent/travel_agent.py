...
# Attempt to import GoogleSearch
try:
    from serpapi import GoogleSearch
except ImportError as e:
    st.error("Error importing GoogleSearch from serpapi. Please check your installation.")
    st.stop()
        ...
        for idx, flight in enumerate(cheapest_flights):
            with cols[idx]:
                ...
                if departure_token:
                    ...
                    booking_options = results_with_booking.get('best_flights', [{}])[0].get('booking_token') if results_with_booking.get('best_flights') else None


                    booking_link = f"https://www.google.com/travel/flights/booking?tfs=ALwJ-W0Qy...{booking_options}" if booking_options else "#"
                    print(booking_link)
                    # Flight card layout
                    st.markdown(
                        f"""
                    ...
                    st.warning("⚠️ Booking link not available for this flight.")
    else:
        st.warning("⚠️ No flight data available.")

    st.subheader("🏨 Hotels & Restaurants")
    st.write(hotel_restaurant_results.content if hasattr(hotel_restaurant_results, 'content') else "No hotel and restaurant information found.")

    st.subheader("🗺️ Your Personalized Itinerary")
    st.write(itinerary.content if hasattr(itinerary, 'content') else "No itinerary generated.")
